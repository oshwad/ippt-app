...
class EMASmoothing @JvmOverloads constructor(
    private val windowSize: Int = EMASmoothing.Companion.DEFAULT_WINDOW_SIZE,
    private val alpha: Float = EMASmoothing.Companion.DEFAULT_ALPHA
) {
    ...
    fun getSmoothedResult(classificationResult: ClassificationResult): ClassificationResult {
        ...
        if (nowMs - lastInputMs > EMASmoothing.Companion.RESET_THRESHOLD_MS) {
            window.clear()
        }
        lastInputMs = nowMs
